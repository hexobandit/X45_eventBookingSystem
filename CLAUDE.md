...
**Registration page** (`admin/edit_registration.html`, styles in `admin/_reg-edit.css`): the attendee's own data on the left (course, name, contact, organization, internal notes, plus a Consent & privacy fold), what the office did with it on the right (status and payment chips with one-click Confirm registration / Record payment, and a Communication card showing which emails went out), and one sticky save bar that also works inside the slide-in panel (`?panel=1`). The quick action buttons write into the real `status` / `payment_status` selects, which sit with every other column inside the "All fields" fold, then submit the same form — so every change goes through `on_model_change` and stays audit-logged, and no column is silently wiped (guarded by `TestFormRoundTrip` in `tests/test_admin_edit_panel.py`). Email and phone stay read-only until the admin confirms the change. Send email / More are dropdowns whose items target hidden sibling forms through the `form=` attribute, because HTML forbids nested forms; in panel mode the email actions are hidden, since they POST to the manage page.
